...
## v2.7.0（当前版本）—— 键盘全面重构，iPhone / iPad 精准等高与适配
- 键盘高度改为分设备/方向的官方等高常量：全面屏 iPhone 竖 336–342、横 211；非全面屏 260/271、横 162；iPad 竖 313/324、横 253/270（12.9 寸略高），含 Home 指示条区域；在 viewWillAppear/viewDidAppear/布局/安全区变化/旋转多时机重算，空数据与未授权状态都不再塌陷或矮一截
- iPad：继续清空系统撤销/重做助理条；regular 宽度下功能内容居中限宽 760pt，避免超宽行；列表底边贴合安全区
- 数据：键盘每次出现先重开共享连接（reloadSync）再读，确保看到主 App 最新落盘记录；未授权时正确清空列表并显示引导
- 结构重构：背景/全宽工具栏/居中 body 容器分层，毛玻璃 + SF Symbols 原生观感；长按菜单（复制并插入/仅复制/在 App 中编辑/删除）与一键点按互不误触，正常路径不跳 App
- 版本 2.7.0 (build 12)

## v2.6.2 —— 修复 v2.6.1 启动闪退（数据库队列重入死锁）
